...
@implementation ChangeTrackingChange
...
{
    NSMutableArray *ret=(NSMutableArray *)callback_ctx;
    if (NULL!=modeTxt && NULL !=parTxt) {
        ChangeTrackingChange *change=[[ChangeTrackingChange alloc] init];
        change.mode=[NSString stringWithUTF8String:(const char*)modeTxt];
        change.par=[NSString stringWithUTF8String:(const char*)parTxt];
...
+(NSArray *)loadChangesFromFile:(NSString *)path
{
    ...
    parseText((xmlChar *)[path UTF8String], paragraph_callback, ret);
